...
import { makeViolation } from '../../src/lib/ruleCatalog.js'

const SEP = '\x1f'
const DAY = 24 * 60 * 60 * 1000

const violation = (severity, rule, branch, message, hint) =>
  makeViolation({ severity, rule, branch, message, hint })
...
export async function checkRules(repo) {
  ...
  for (const b of branches) {
    ...
    const isFlowBranch = ['feature', 'release', 'hotfix', 'support'].includes(kind)
    ...
    //    자동 생성 브랜치(dependabot/* 등)까지 검사하면 경고만 늘어난다
